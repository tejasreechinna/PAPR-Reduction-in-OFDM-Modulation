...
clear all;
close all;
 L=input('Enter the L factor(1 to 1.5)= '); %1.25
 N=input('Enter the number of transmitted symbols(Power of 2)(preferably>32)='); %128
M=input('Enter the alphabet size(Power of 2 and less than number of Symbols)(preferably<32)='); %16
...
x=ifft(aa);  
plot(x); 
title("OFDM modulated signal in frquency domain");
figure;
x_mag=abs(x);

subplot(3,1,1),stem(x_mag),title('OFDM without Clipping signal');
xlim([0 LN]);
